"""
Analyze recent mech delivers for a specific mech address.

Fetches delivers from the mech-marketplace subgraph, enriches with IPFS data,
and produces a summary analysis (tool breakdown, error rates, response patterns).

Usage:
    python analyze_mech_delivers.py <mech_address> --period 3h
    python analyze_mech_delivers.py <mech_address> --period 7d
    python analyze_mech_delivers.py <mech_address> --period 3h --json
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze recent mech delivers."
    )
    parser.add_argument("mech", help="Mech contract address (0x...)")
    parser.add_argument(
        "--period",
        default="3h",
        help="Look-back period, e.g. 3h, 7d (default: 3h)",
    )
    parser.add_argument(
        "--json",
        action="store_true",
...
def main():
    ...
    print(f"Fetching delivers since {_ts_to_str(str(start_ts))}...")
