"""
Mech usage timeline script.

Directly queries all mech requests in a user-specified time window from the
mech-marketplace subgraph, bins by time, and plots per-mech request-count
trend lines over time.

No bet data is needed — mech requests are queried directly.

Usage:
    python mech_usage_timeline.py                        # last 30 days
    python mech_usage_timeline.py --period 7d            # last 7 days
    python mech_usage_timeline.py --period 90d           # last 90 days
    python mech_usage_timeline.py --start 2025-12-01 --end 2026-01-01
    python mech_usage_timeline.py --start 2025-12-01     # from date to now

Cache:
    .mech_usage_timeline_cache.json (next to this script, TTL 12 hours)
...
_CACHE_FILE = Path(__file__).parent / ".mech_usage_timeline_cache.json"

MIN_REQUESTS_FOR_LINE = 2  # minimum data points before a mech line is drawn
...
# ---------------------------------------------------------------------------
# Mech request fetching (direct, cached)
# ---------------------------------------------------------------------------
...
def _fetch_mech_requests_from_api(
    start_ts: int, end_ts: int, cache_key: str, batch_size: int = 1000
) -> list[dict]:
    """
    Fetch all mech requests in [start_ts, end_ts] directly from the subgraph.
    Checkpoints partial results to the cache every _CHECKPOINT_EVERY batches so
    a restart after a failure can resume rather than start from scratch.
    """
    # Resume from a previous partial fetch if available
    entry = _cache.get(cache_key)
    if entry and not entry.get("complete", True):
        all_requests: list[dict] = entry["requests"]
        skip = entry["skip"]
        print(
            f"    Resuming from checkpoint: {len(all_requests)} requests already fetched (skip={skip})..."
        )
    else:
        all_requests = []
        skip = 0
    ...
    while True:
        ...
        response = _post_with_retry(
            OLAS_MECH_SUBGRAPH_URL,
            json={"query": GET_MECH_REQUESTS_QUERY, "variables": variables},
            headers={"Content-Type": "application/json"},
        )
        data = response.json()
        if "data" not in data:
            raise RuntimeError(f"Subgraph error (mech requests): {data}")
        batch = (data.get("data") or {}).get("requests") or []
        if not batch:
            break
        ...
        if batch_num % _CHECKPOINT_EVERY == 0:
            print(f"    Fetched {len(all_requests)} requests so far (checkpointing)...")
            _cache[cache_key] = {
...
def fetch_mech_requests_in_range(start_ts: int, end_ts: int) -> list[dict]:
    """
    Returns all mech requests in [start_ts, end_ts], using a disk-backed cache.
    Re-fetches if the cache entry is missing, stale, or incomplete.
    Resumes a previous incomplete fetch if one exists.
    """
    cache_key = f"requests:{start_ts}:{end_ts}"
    entry = _cache.get(cache_key)
    ...
    if entry and not entry.get("complete", True):
        ...
        print(
            f"  Fetching mech requests from subgraph "
            f"({_ts_to_date(start_ts)} → {_ts_to_date(end_ts)})..."
        )

    reqs = _fetch_mech_requests_from_api(start_ts, end_ts, cache_key)
    _cache[cache_key] = {
        "fetched_at": int(time.time()),
        "requests": reqs,
        "complete": True,
...
def bin_requests_by_mech(
    requests: list[dict], start_ts: int, end_ts: int
) -> tuple[list[datetime], dict[str, list[int]]]:
    """
    Groups mech requests into time bins and counts per mech per bin.

    Returns:
        bin_labels  – list of datetime objects (one per bin)
        mech_series – dict mapping mech address -> list of request counts per bin
    """
    ...
    for req in requests:
        ...
        mech = (req.get("priorityMech") or req.get("deliveredByMech") or "unknown").lower()
        bin_idx = None
...
def plot_mech_usage_timeline(
    ...
    for mech, counts in mech_series.items():
        ...
        if len(non_zero) < MIN_REQUESTS_FOR_LINE:
            continue
        xs, ys = zip(*non_zero)
    ...
    ax.set_xlabel("Date")
    ax.set_title(
        f"Mech Usage Over Time  |  " f"{_ts_to_date(start_ts)} → {_ts_to_date(end_ts)}"
    )
...
def compute_mech_stats(requests: list[dict]) -> list[dict]:
    """Per-mech request counts and tool breakdown across the entire period."""
    totals: dict[str, int] = defaultdict(int)
    tools_seen: dict[str, set[str]] = defaultdict(set)

    for req in requests:
        mech = (req.get("priorityMech") or req.get("deliveredByMech") or "unknown").lower()
        totals[mech] += 1
        tool = (req.get("parsedRequest") or {}).get("tool")
        if tool:
            tools_seen[mech].add(tool)
        ...
        stats.append(
            {
                "mech": mech,
                "total": total,
                "tools": sorted(tools_seen.get(mech, set())),
            }
        )
...
def print_summary(stats: list[dict], start_ts: int, end_ts: int) -> None:
    mech_w = max((len(s["mech"]) for s in stats), default=4)
    mech_w = max(mech_w, 4)
    header = f"{'Mech':<{mech_w}} | {'Total':>7} | Tools"
    sep = "-" * (len(header) + 20)

    print(f"\nMech usage summary  ({_ts_to_date(start_ts)} → {_ts_to_date(end_ts)})")
    print(sep)
    print(header)
    print(sep)
    for s in stats:
        tools_str = ", ".join(s["tools"]) if s["tools"] else "—"
        print(f"{s['mech']:<{mech_w}} | {s['total']:>7} | {tools_str}")
    print(sep)

    known = [s for s in stats if s["mech"] != "unknown"]
    if known:
        total_k = sum(s["total"] for s in known)
        print(f"\nTotal requests (known mechs): {total_k}")

    unk = next((s for s in stats if s["mech"] == "unknown"), None)
    if unk:
        print(f"Requests with no priorityMech: {unk['total']}")


def print_unknown_tool_breakdown(requests: list[dict]) -> None:
    """Print a tool-usage breakdown for requests where mech is unknown."""
    ...
    tool_w = max(len(t) for t, _ in sorted_tools)
    tool_w = max(tool_w, 4)

    print(f"\nTool breakdown for unknown-mech requests ({total_unknown} total)")
    sep = "-" * (tool_w + 22)
    print(sep)
    print(f"{'Tool':<{tool_w}} | {'Count':>7} | {'Share':>6}")
    print(sep)
    for tool, count in sorted_tools:
        share = count / total_unknown * 100
        print(f"{tool:<{tool_w}} | {count:>7} | {share:>5.1f}%")
    print(sep)
...
def _parse_args() -> tuple[int, int]:
    parser = argparse.ArgumentParser(
        description="Plot per-mech request counts over a time period."
    )
...
def main() -> None:
    ...
    print(
        f"\nMech Usage Timeline: {_ts_to_date(start_ts)} → {_ts_to_date(end_ts)}"
        f" ({(end_ts - start_ts) // 86400} days)"
    )
    print("=" * 60)

    print("\n[1/3] Fetching mech requests...")
    requests_data = fetch_mech_requests_in_range(start_ts, end_ts)
    if not requests_data:
        print("No mech requests found for the selected time range. Exiting.")
        sys.exit(0)
    unique_mechs = len(
        {r.get("priorityMech") for r in requests_data if r.get("priorityMech")}
    )
    print(f"  {len(requests_data)} requests across {unique_mechs} unique mechs.")

    print("\n[2/3] Computing statistics...")
    stats = compute_mech_stats(requests_data)
    print_summary(stats, start_ts, end_ts)
    print_unknown_tool_breakdown(requests_data)

    bin_labels, mech_series = bin_requests_by_mech(requests_data, start_ts, end_ts)
    span_days = (end_ts - start_ts) / 86400
    granularity = "daily" if span_days <= 30 else "weekly"
    print(f"\n  Binned into {len(bin_labels)} {granularity} bins.")

    print("\n[3/3] Plotting chart...")
    plot_mech_usage_timeline(bin_labels, mech_series, start_ts, end_ts)
